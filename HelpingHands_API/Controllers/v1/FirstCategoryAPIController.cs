       ...
       // [Authorize(Roles = "admin")]
        ...
        public async Task<ActionResult<APIResponse>> CreateFirstCategory([FromBody] FirstCategoryCreateDTO createDTO)
        {
            ...
            try
            {
                ...
                //}
                if (await _unitOfWork.FirstCategory.GetAsync(u => u.FirstCategoryName.ToLower() == createDTO.FirstCategoryName.ToLower()) != null)
                {
                    ModelState.AddModelError("ErrorMessages", "FirstCategory already Exists!");
                    return BadRequest(ModelState);
                }
